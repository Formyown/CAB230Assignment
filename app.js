...
app.get("/search", function(req, resp) {
    ...
    req.db.query(
        "select offence_columns.column from offence_columns where pretty = ?",
        [req.query.offence],
        function(err, result) {
            if (err) {
                resp.status(500).josn(err);
            ...
            var query =
                'select count(o.id) as "count", o.area, a.lat, a.lng from offences o, areas a where ';
            ...
            query +=
                where.map(i => i[0]).join(" and ") +
                " group by o.area, a.lat, a.lng";
            console.log("Search sql: " + query);
            var args = [];
            where.forEach(i => args.push(i[1]));
